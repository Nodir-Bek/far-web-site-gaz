...
<template>
  <div
    class="relative bg-white flex items-center justify-between gap-3 w-full h-12 border rounded-lg border-secondary-bold px-4 py-2"
  >
    <Icon
    ...
    <input
      type="text"
      class="w-full h-full border-none outline-none focus:outline-none"
      v-model="searchVal"
      :placeholder="$t('Qidiruv')"
    />
    <button
      @click="handleClear"
      class="text-brand-color flex justify-center items-center transition-all duration-300 ease-in-out"
      :class="searchVal === '' ? 'scale-0' : 'scale-100'"
      aria-label="Clear"
    >
      <Icon name="icon-park-outline:close" class="text-brand-color" size="20" />
    </button>

    <div
      v-if="searchVal.length > 0 && showResults"
      class="flex justify-start items-start absolute w-screen max-w-[calc(100vw-12%)] sm:max-w-full top-20 left-0 max-h-[calc(100vh-250px)] bg-white rounded-lg p-5 overflow-y-auto shadow-xl"
    >
      <div
        class="w-full h-full flex justify-center items-center overflow-y-auto snap-mandatory"
      >
        <LoadingDots v-if="loading" size="large" />
        <div v-else class="w-full h-full flex flex-col gap-4 px-4">
          <div
            v-if="results.length === 0 && searchVal.length > 0"
            class="w-full h-full flex flex-col justify-center items-center text-[#535a70] text-xl font-semibold"
          >
            <svg
              class="svg-icon w-full max-w-[200px] h-full max-h-[300px]"
              style="
          ...
          <div
            ...
            :class="{
              'border-b border-secondary-bold/15': index !== results.length - 1,
            }"
            :key="item._id"
          >
            <div class="flex items-center text-[#535a70]">
            ...
            <NuxtLink
              ...
              class="w-full text-xl text-brand-color hover:underline font-bold line-clamp-2"
            >
              {{ item.title }}
            </NuxtLink>
            <p class="text-secondary-light line-clamp-2 text-base">
              {{ item.desc }}
            </p>
          </div>
        </div>
