...
import { documentsService, personnelPoliciesService } from "~/services/pages";
definePageMeta({
  title: "personnel_policy",
  desc: "personnel_policy",
});
...
const { data: documents } = await personnelPoliciesService.getAllWithPaginate(query);
</script>
...
<template>
  <div class="w-full">
    ...
    >
      <div class="flex flex-col gap-[12px] sm:gap-[16px] lg:gap-[24px]">
        <h2
        ...
        <p
          class="text-16px leading-25px font-normal text-main-text-color text-justify"
        >
          {{ $t("personnel_policy_desc") }}
        </p>
      </div>
